...
//   • Vision (disease + crop id) → qwen/qwen3.6-27b  (image + text inputs)
//   • AgroBot chat              → llama-3.3-70b-versatile
...
class DetectionService extends ChangeNotifier {
  ...
  static const String _groqApiKey =
      String.fromEnvironment('GROQ_API_KEY', defaultValue: 'YOUR_GROQ_API_KEY');
  static const String _groqEndpoint =
      'https://api.groq.com/openai/v1/chat/completions';

  // Confirmed active Groq vision model (supports text + image inputs)
  ...
  Future<String> identifyCrop(Uint8List bytes) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            'identifyCrop Groq error ${response.statusCode}: ${response.body}');
        return 'Unknown';
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _callGroqVision(
    ...
    final b64 = base64Encode(bytes);
    debugPrint(
        'Groq vision request: for crop $cropType, b64 length ${b64.length}');

    final prompt = '''
Analyze this crop leaf image for diseases.
${cropType != 'Auto-detect' ? 'Crop: $cropType.' : 'Identify the crop first.'}

Respond ONLY with a compact JSON object (no markdown, no extra text):
{"cropType":"string","diseaseName":"string","severity":"low|moderate|high|critical","confidenceScore":0.0,"severityPercent":0.0,"description":"1-2 sentences","symptoms":["s1","s2"],"treatments":["t1","t2"],"preventions":["p1","p2"],"weatherNote":"string"}''';
    ...
    try {
      ...
      // 1. Remove reasoning tags <think>...</think> produced by reasoning models (e.g., Qwen/DeepSeek)
      String clean = text
          .replaceAll(
              RegExp(r'<think>[\s\S]*?</think>', caseSensitive: false), '')
          .trim();
      ...
      // 3. Extract JSON object substring {...} if surrounding text exists
      ...
      try {
        ...
        debugPrint('Failed to parse clean JSON: "$clean". Error: $e');
        return {
          'cropType': cropType == 'Auto-detect' ? 'Crop' : cropType,
          'diseaseName': 'Health Inspection',
          'severity': 'low',
          'confidenceScore': 0.8,
          'severityPercent': 10.0,
          'description': clean.replaceAll(RegExp(r'<[^>]*>'), '').trim(),
          'symptoms': ['Visual inspection completed'],
          'treatments': ['Monitor crop health regularly'],
          'preventions': ['Ensure proper irrigation and soil nutrients'],
          'weatherNote': 'Keep crop dry during humid conditions',
        };
      }
    } catch (e) {
